...
----------------------------------------
## Novelty No.2
## gridworld-v2: add a new type of tree to the env, the agent has can get more logs by breaking it.
## Trees, Rubber_tree, rock, crafting_table, New_tree
...
class NovelGridworldV2Env(NovelGridworldV0Env):
    ...
    def __init__(self, map_width=None, map_height=None, items_id=None, items_quantity=None, initial_inventory = None, goal_env = None, is_final = False):
        ...
        self.items = ['wall', 'tree', 'rock', 'rubber_tree', 'crafting_table', 'pogo_stick', 'oak_tree']
        self.items_id = self.set_items_id(self.items)
        self.items_quantity = {'tree': 6, 'rock': 2, 'rubber_tree' : 1, 'crafting_table': 1, 'pogo_stick': 0, 'oak_tree': 1,}
        ...
        self.items_lidar = ['wall', 'tree', 'rock', 'rubber_tree', 'crafting_table', 'oak_tree']
    ...
    def step(self, action):
        ...
        """
        ### the New_tree: can provide 5 logs (may be 3, the number might be changed)
        ### the movment won't be influenced, only the break action can be changed
        ### for the product provided by New_tree is same as the tree, the inventory_quantity is added to the tree
        ##################################################### Jinghan

        # Michael - renaming to oak_tree (for semantic similarity), adding oak_tree as
        # possible inventory item, and changing crafting pogo_stick to depend on
        # tree + oak_tree >= 3 in inventory.
        #############################
        ...
        if action == 0: # forward
            ...
            if self.block_in_front_str == 'tree' or self.block_in_front_str == 'rock' or self.block_in_front_str == 'rubber_tree' or self.block_in_front_str == 'oak_tree':
                block_r, block_c = self.block_in_front_location
                self.map[block_r][block_c] = 0
                #<<<< CHANGED >>>>#
                # if (self.block_in_front_str == 'tree' or self.block_in_front_str == 'oak_tree' and self.inventory_items_quantity['tree'] <= 2) or (self.block_in_front_str == 'rock' and self.inventory_items_quantity['rock'] <= 1) or (self.block_in_front_str == 'rubber_tree' and self.inventory_items_quantity['rubber_tree'] <= 0):
                if ((self.block_in_front_str == 'tree' or self.block_in_front_str == 'oak_tree') and (self.inventory_items_quantity['tree'] + self.inventory_items_quantity['oak_tree']) <= 2) or (self.block_in_front_str == 'rock' and self.inventory_items_quantity['rock'] <= 1) or (self.block_in_front_str == 'rubber_tree' and self.inventory_items_quantity['rubber_tree'] <= 0):
                    reward = self.reward_break
                else:
                    reward = 0
                if self.block_in_front_str=='oak_tree':
                    self.inventory_items_quantity['oak_tree'] += 5
                else:
            ...
            if self.block_in_front_str == 'crafting_table':
                # if (self.inventory_items_quantity['tree'] >= 3) and self.inventory_items_quantity['rock'] >= 2 and self.inventory_items_quantity['rubber_tree'] >= 1 : # recipe: 3logs+2stone+1rubber = 1 pogostick
                if (self.inventory_items_quantity['tree'] + self.inventory_items_quantity['oak_tree'] >= 3) and self.inventory_items_quantity['rock'] >= 2 and self.inventory_items_quantity['rubber_tree'] >= 1 : # recipe: 3logs+2stone+1rubber = 1 pogostick
