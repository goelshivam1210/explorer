...
class NovelGridworldV2Env(gym.Env):
    ...
    def __init__(self, map_width=None, map_height=None, items_id=None, items_quantity=None, initial_inventory = None, goal_env = None, is_final = False):
        ...
        ### 这个要改
        ############################
        self.items = ['wall', 'tree', 'rock', 'rubber_tree', 'crafting_table','New_tree', 'pogo_stick']
        ...
        ################################
        self.items_quantity = {'tree': 6, 'rock': 2, 'rubber_tree' : 1, 'crafting_table': 1, 'New_tree':1, 'pogo_stick': 0}
        ...
        ### wall是边界，不是obj
        ###########################################
        self.items_lidar = ['wall', 'crafting_table', 'tree', 'rock', 'rubber_tree','New_tree']
    ...
    def step(self, action):
        ...
        """
        ### the New_tree: can provide 3 logs (the number might be changed)
        ### the movment won't be inflenced, only the break action can be changed
        ...
        if action == 0: # forward
            ...
            if self.block_in_front_str == 'tree' or self.block_in_front_str == 'rock' or self.block_in_front_str == 'rubber_tree' or self.block_in_front_str == 'New_tree':
                block_r, block_c = self.block_in_front_location
                self.map[block_r][block_c] = 0
                if (self.block_in_front_str == 'tree' or self.block_in_front_str == 'New_tree' and self.inventory_items_quantity['tree'] <= 2) or (self.block_in_front_str == 'rock' and self.inventory_items_quantity['rock'] <= 1) or (self.block_in_front_str == 'rubber_tree' and self.inventory_items_quantity['rubber_tree'] <= 0):
                    reward = self.reward_break
                else:
                    reward = 0
                if self.block_in_front_str=='New_tree':
                    self.inventory_items_quantity['tree'] += 5
                else:
